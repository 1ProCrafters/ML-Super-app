# algorithms/regression/lasso_regression.py
...
class LassoRegression:
    ...
    def fit(self, X, y):
        """
        Train the Lasso Regression model using Gradient Descent with L1 regularization.
        ...
        for epoch in range(self.epochs):
            ...
            for i in range(n_features):
                if i == 0:
                    ...
                    gradient = sum(e * x[i] for e, x in zip(errors, X)) / n_samples
                else:
                    gradient = (sum(e * x[i] for e, x in zip(errors, X)) + self.alpha * (1 if self.weights[i] > 0 else -1)) / n_samples
